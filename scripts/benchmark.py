...
for case in cases:
    try:
        ...
        passed = all(
            out.get(k) and out[k].startswith(case[k])
            for k in ["start", "end", "rrule"]
            if k in case
        ) and (bool(out["warnings"]) if case.get("review") else True)
        results.append(
            {
                "text": case["text"],
                "passed": bool(passed),
        ...
        results.append({"text": case["text"], "passed": False, "error": str(exc)})
report = {
    "reference": "2026-09-03T10:00:00+05:00",
    "cases": len(results),
