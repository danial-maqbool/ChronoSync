...
class EventInput(BaseModel):
    ...
    def valid_reminders(cls, v):
        if any(n < 0 or n > 525600 for n in v):
            raise ValueError("Reminder offsets must be between 0 and 525600 minutes")
    ...
    def times(self):
        ...
        if self.rrule:
            ...
            if (
                len(self.rrule) > 300
                or not re.fullmatch(r"[A-Z0-9=;,+-]+", self.rrule)
                or not self.rrule.startswith("FREQ=")
            ):
            ...
            if any(
                ...
                raise ValueError("Recurrence must be daily or less frequent")
...
class RuleInput(BaseModel):
    ...
    def validate_rule(self):
        if not set(self.conditions) <= {"contains", "type", "tags", "source"}:
            raise ValueError("Unsupported rule condition")
        if not set(self.actions) <= {"importance", "tags", "reminders"}:
