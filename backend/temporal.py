...
DATE_PATTERN = re.compile(
    ...
    + MONTH
    + r"\s+\d{1,2}(?:st|nd|rd|th)?(?:,?\s+\d{4})?|\d{1,2}(?:st|nd|rd|th)?\s+"
    + MONTH
    + r"(?:\s+\d{4})?|day after tomorrow|tomorrow|today|yesterday|(?:next\s+)?"
    + DAY
    + r"|in\s+(?:\d+|"
    + "|".join(NUMS)
...
def resolve(text, reference, settings):
    ...
    if re.search(
        r"\b(sometime|around|approximately|maybe|possibly)\b|\bnext (week|month)\b",
        lower,
    ) and not re.search(
        r"(beginning|end|first|last) of|first business day|last working day|first "
        + DAY,
        lower,
    ):
        warnings.append("Date is not sufficiently specific.")
        return out
    explicit_zone = re.search(
        r"\b(UTC|GMT|Asia/[A-Za-z_]+|Europe/[A-Za-z_]+|America/[A-Za-z_]+)\b", text
    )
    source_zone = zone
    if explicit_zone:
        ...
        try:
            ...
            warnings.append("Unresolved source timezone")
            return out
    elif re.search(r"\b(?:EST|CST|PST|IST|BST|EDT|PDT)\b", text):
        warnings.append("Timezone abbreviation is ambiguous; choose an IANA timezone.")
        return out
    # Corrections and reschedules choose the target, never the superseded date.
    working = lower
    moved = re.search(
        r"(?:moved (?:from .+? )?to|rescheduled to|postponed until|changed to|make it|instead(?! of),?)\s+(.+)",
        lower,
    )
    if moved:
        working = moved[1]
        reasons.append("Selected the corrected or rescheduled target.")
    elif "instead of" in lower:
        working = lower.split("instead of")[0]
    elif re.search(r"\bnot\b|no longer", lower):
        correction = re.search(
            r"(?:not|no longer)\s+(?:on\s+)?" + DAY + r"[.,; -]+(?:it is\s+)?(.+)",
            lower,
        )
        if correction:
            working = correction[1]
            reasons.append("Excluded the negated date.")
        else:
            warnings.append("Negated date; no event should be scheduled.")
            return out
    ...
    business = re.search(
        r"(first business day|last working day|end of (?:next|this) month|beginning of next week)",
        working,
    )
    ...
    if ordinal:
        ...
        if phrase == "beginning of next week":
            ...
            date = base.replace(
                day=1
                if phrase == "first business day"
                else calendar.monthrange(base.year, base.month)[1]
            )
            if phrase in {"first business day", "last working day"}:
                while date.weekday() > 4:
                    date += timedelta(days=1 if phrase == "first business day" else -1)
                reasons.append(
                    "Business days mean Monday–Friday; public holidays are not inferred."
                )
    match = DATE_PATTERN.search(working)
    if match and date is None:
        expr = match[0]
        out["temporal_expression"] = expr
        if expr in {"today", "tomorrow", "day after tomorrow", "yesterday"}:
            ...
            day = expr.split()[-1]
            delta = (DAYS.index(day) - ref.weekday()) % 7
            if expr.startswith("next "):
                delta = (7 - ref.weekday()) + DAYS.index(day)
                reasons.append(
                    "Next weekday means that weekday in the next Monday-starting week."
                )
            date = ref + timedelta(days=delta)
        elif expr.startswith("in "):
            count = int(expr.split()[1])
            unit = expr.split()[2]
            date = ref + (
                relativedelta(months=count)
                if unit.startswith("month")
                else timedelta(days=count * (7 if unit.startswith("week") else 1))
            )
        else:
            date = dateparser.parse(
                ...
                settings={
                    ...
                    "DATE_ORDER": (
                        ...
                        else settings.get("date_order", "DMY")
                    ),
            ...
            if date:
                ...
                if not re.search(r"\b\d{4}\b", expr):
                    reasons.append(
                        "Year inferred from the source reference year; no automatic one-year rollover."
                    )
                if re.fullmatch(r"\d{1,2}/\d{1,2}(?:/\d{2,4})?", expr):
                    reasons.append(
                        "Numeric date interpreted using configured "
    ...
    if rng:
        month = next(
            ...
            if m.lower().startswith(rng[1][:3].lower())
        )
        try:
            date = ref.replace(month=month, day=int(rng[2]))
            end_date = date.replace(day=int(rng[3]))
        except ValueError:
        ...
        if len(matches) >= 2 and date:
            other = resolve(
                matches[-1][0], date.isoformat(), {**settings, "timezone": source_zone}
            )
            if other["start"]:
    ...
    if recurring:
        ...
        if "weekday" in working:
            freq = "WEEKLY"
            extra = ";BYDAY=MO,TU,WE,TH,FR"
            date = date or ref
            while date.weekday() > 4:
                date += timedelta(days=1)
        elif "first" in working and wk:
            freq = "MONTHLY"
            extra = ";BYDAY=1" + wk[0][:2].upper()
            date = ref.replace(day=1)
            date += timedelta(days=(DAYS.index(wk[0]) - date.weekday()) % 7)
            if date.date() < ref.date():
                date = month_shift.replace(day=1)
                date += timedelta(days=(DAYS.index(wk[0]) - date.weekday()) % 7)
        elif wk:
            freq = "WEEKLY"
            extra = ";BYDAY=" + wk[0][:2].upper()
        elif re.search(r"(?:2|two) weeks", working):
            freq = "WEEKLY"
            extra = ";INTERVAL=2"
        elif "month" in working:
            freq = "MONTHLY"
            daymatch = re.search(r"on the (\d{1,2})", working)
            if daymatch:
                day = int(daymatch[1])
                extra = ";BYMONTHDAY=" + str(day)
                try:
                    date = ref.replace(day=day)
                    if date.date() < ref.date():
                        date = date + relativedelta(months=1)
                except ValueError:
                    warnings.append("Invalid monthly day")
                    return out
        elif "annually" in working:
            freq = "YEARLY"
        out["rrule"] = "FREQ=" + freq + extra
        if date is None:
            warnings.append("Recurrence start date must be selected.")
            return out
    if date is None:
        warnings.append("No specific calendar date could be resolved.")
        return out
    date = date.replace(hour=0, minute=0, second=0, microsecond=0)
    clock = re.search(
        r"\b(?:at\s+)?(\d{1,2})(?::(\d{2}))?\s*(a\.?m\.?|p\.?m\.?)\b", working
    )
    ...
    if clock:
        ...
        if not 1 <= hour <= 12 or minute > 59:
            warnings.append("Invalid clock time")
            return out
        time = (hour % 12 + (12 if clock[3].startswith("p") else 0), minute)
    elif military:
        time = (int(military[1]), int(military[2]))
    elif "noon" in working:
        time = (12, 0)
    elif "midnight" in working:
        time = (0, 0)
    elif bare:
        warnings.append("AM/PM is missing; choose an exact time.")
    business_time = (
        "eod"
        if re.search(r"\beod\b|end of day", working)
        else ("cob" if re.search(r"\bcob\b|close of business", working) else None)
    )
    if business_time:
        configured = settings.get(business_time)
        if configured:
            time = tuple(map(int, configured.split(":")))
            reasons.append("Used configured " + business_time.upper() + " time.")
        else:
            warnings.append(business_time.upper() + " time is not configured.")
    if (
        time is None
        and not warnings
        and settings.get("default_deadline_time")
        and re.search(r"due|deadline|submit|by\b", working)
    ):
        time = tuple(map(int, settings["default_deadline_time"].split(":")))
        reasons.append("Time defaulted from deadline settings.")
    if time:
        try:
        ...
        if not datetime_exists(date) or datetime_ambiguous(date):
            warnings.append(
                "DST transition makes this local time nonexistent or ambiguous."
            )
            return out
        end = date + timedelta(minutes=settings.get("duration", 60))
        reasons.append(
            "End uses configured default duration unless an explicit end is present."
        )
        clocks = list(re.finditer(r"\b(\d{1,2})(?::(\d{2}))?\s*(am|pm)\b", working))
        if (
            len(clocks) >= 2
            and re.search(r"\b(?:to|until)\b|\s-\s", working)
            and not moved
        ):
            c = clocks[-1]
            end = date.replace(
                hour=int(c[1]) % 12 + (12 if c[3] == "pm" else 0), minute=int(c[2] or 0)
            )
            if end <= date:
                warnings.append(
                    "End time is not after start; confirm overnight intent."
                )
    else:
        out["all_day"] = not warnings
        end = (end_date or date).replace(
            hour=0, minute=0, second=0, microsecond=0
        ) + timedelta(days=1)
        reasons.append(
            "Date only; exact time is unknown. All-day calendar representation requires approval."
        )
    if end_date and time:
        end = end_date.replace(hour=end.hour, minute=end.minute)
    if end <= date:
        warnings.append("End must be after start.")
    if date.date() < ref.date():
        warnings.append("PAST_DATE_WARNING")
