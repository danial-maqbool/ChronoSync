...
if __name__=='__main__':
    ...
    if venv.exists() and Path(sys.executable).resolve()!=venv.resolve():
        raise SystemExit(subprocess.call([str(venv),str(root/'run.py'),*sys.argv[1:]]))
    if not (root/'frontend/dist/index.html').exists():
        print('Build the frontend first: cd frontend && npm install && npm run build')
        raise SystemExit(1)
    import uvicorn
    from backend.app import create_app
    uvicorn.run(create_app(),host='127.0.0.1',port=int(os.environ.get('CHRONOSYNC_PORT','8765')))
