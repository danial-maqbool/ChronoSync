...
def test_context():
    ...
    assert extract(
        source("The meeting is not Monday. It is Wednesday."), DEFAULTS, [], []
    )[0]["start"].startswith("2026-09-09")


def test_change_duplicate_conflict():
    e = extract(source("Interview Tuesday at 3 PM."), DEFAULTS, [], [])[0]
    e["id"] = "old"
    n = extract(source("Interview moved to Thursday at 4 PM."), DEFAULTS, [e], [])[0]
    assert n["related_ids"] == ["old"] and n["change_kind"] == "RESCHEDULE"
    assert extract(source("Interview Tuesday at 3 PM."), DEFAULTS, [e], [])[0][
        "duplicate_ids"
    ] == ["old"]
    n = {**e, "id": "new"}
...
def test_calendar(tmp_path):
    s = Store(tmp_path)
    s.migrate()
    p = MockCalendarProvider(s)
    e = extract(source("Meeting tomorrow at 4 PM."), DEFAULTS, [], [])[0]
    e.update(id="test", status="CONFIRMED")
    ident = p.create_event(e)
    e["title"] = "Changed"
    p.update_event(ident, e)
    ...
    assert p.get_event(ident) is None
